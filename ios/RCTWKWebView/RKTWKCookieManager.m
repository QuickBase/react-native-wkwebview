...
#import "RKTWKCookieManager.h"

@import WebKit;

@implementation RKTWKCookieManager

RCT_EXPORT_MODULE(WKCookieManager);
...
{
    // If device is iOS10 or below still clear NSHTTPCookieStorage because we fall back to it
    if (!@available(iOS 11, *)) {
    ...
    // Clear cookies from WKWebViews on iOS9+
    if (@available(iOS 9, *)) {
